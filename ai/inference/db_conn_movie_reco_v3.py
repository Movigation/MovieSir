...
Changes from V2:
- Removed unused imports: combinations, sqrt
- Removed unused variables: ott_id_to_name, all_otts, all_genres, lightgcn_idx_to_movie
- Removed unused method: _calculate_rating_score()
- Removed unused parameter: movie_ids in _apply_filters()
"""
...
class HybridRecommenderV3:
    ...
    def recommend_single(
        ...
        if candidates:
            ...
            rec_type_label = '🔀 하이브리드' if rec_type == 'hybrid' else '📖 SBERT만'
            print(f"✅ [{rec_type_label}] {selected['title']} ({selected['runtime']}분, score={selected.get('score', 0):.3f})")
            return selected
        ...
        if under_time:
            ...
            rec_type = closest.get('recommendation_type', 'unknown')
            rec_type_label = '🔀 하이브리드' if rec_type == 'hybrid' else '📖 SBERT만'
            print(f"✅ (fallback) [{rec_type_label}] {closest['title']} ({closest['runtime']}분, score={closest.get('score', 0):.3f})")
            return closest
